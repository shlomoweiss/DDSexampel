# Windows PowerShell script to package ICDWrapperCpp binaries with Conan
...
Write-Host "PASSED: Found ICDWrapper.dll: $(Get-Item $dllPath | Select-Object -ExpandProperty Length) bytes" -ForegroundColor Green
Write-Host "PASSED: Found ICDWrapper.lib: $(Get-Item $libPath | Select-Object -ExpandProperty Length) bytes" -ForegroundColor Green
...
try {
    $packageList = conan list "icdwrappercpp/*"
    if ($LASTEXITCODE -eq 0) {
...
Write-Host "SUCCESS: ICDWrapperCpp Conan package created!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
...
Write-Host "       self.requires('icdwrappercpp/1.0.0')" -ForegroundColor Gray
Write-Host ""
Write-Host "2. Add to CMakeLists.txt:" -ForegroundColor White
Write-Host "   find_package(icdwrappercpp REQUIRED)" -ForegroundColor Gray
Write-Host "   target_link_libraries(your_target icdwrappercpp::icdwrappercpp)" -ForegroundColor Gray

Write-Host "`nPackage Details:" -ForegroundColor Yellow
try {
    conan list "icdwrappercpp/1.0.0:*"
} catch {
...
if ($Publish) {
    ...
    try {
        ...
        conan upload "icdwrappercpp/1.0.0" --remote=local-conan --confirm
        if ($LASTEXITCODE -eq 0) {
