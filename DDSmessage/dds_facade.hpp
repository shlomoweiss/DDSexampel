...
// Write a HelloWorld sample with index + message. Returns 1 on success.
ICD_API int dds_write(uint32_t index, const char* message);

// Try take one sample. If a sample is available, fills outputs and returns 1; otherwise returns 0.
