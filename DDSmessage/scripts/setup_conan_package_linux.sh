#!/bin/bash
# Linux Bash script to package DDSmessage binaries with Conan
...
echo "========================================"
echo "DDSmessage Conan Packaging (Linux)"
echo "========================================"
...
if [ -d "$BUILD_DIR" ]; then
    SO_FILE=$(find "$BUILD_DIR" -name "libICD.so*" | head -n 1)
fi

# If not found in build_linux, check regular build directory
if [ -z "$SO_FILE" ] && [ -d "build" ]; then
    SO_FILE=$(find "build" -name "libICD.so*" | head -n 1)
    if [ -n "$SO_FILE" ]; then
        BUILD_DIR="build"
    fi
fi
...
if [ -z "$SO_FILE" ]; then
    echo "FAILED: libICD.so not found in build_linux/ or build/"
...
if conan list "ddsmessage/*" >/dev/null 2>&1; then
    echo "PASSED: Package verification:"
    conan list "ddsmessage/*"
else
...
echo ""
echo "========================================"
echo "SUCCESS: DDSmessage Conan package created!"
echo "========================================"
...
echo "       self.requires('ddsmessage/1.0.0')"
echo ""
echo "2. Add to CMakeLists.txt:"
echo "   find_package(ddsmessage REQUIRED)"
echo "   target_link_libraries(your_target ddsmessage::ddsmessage)"

echo ""
echo "Package Details:"
conan list "ddsmessage/1.0.0:*" || echo "Could not display package details"
...
if [ "$PUBLISH" = true ]; then
    ...
    if conan upload "ddsmessage/1.0.0" --remote=local-artifactory --confirm; then
